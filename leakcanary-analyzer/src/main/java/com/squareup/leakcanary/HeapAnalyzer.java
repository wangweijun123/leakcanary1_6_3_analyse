...
public final class HeapAnalyzer {
  ...
  public @NonNull AnalysisResult checkForLeak(@NonNull File heapDumpFile,
    ...
    try {
      ...
      Log.d("LeakCanary", "findLeakingReference ... referenceKey："+referenceKey);

      // 第一步: 根据key 找到 KeyedWeakReference 实例Instance


      // 第二步:找到引用链

      Instance leakingRef = findLeakingReference(referenceKey, snapshot);
      Log.d("LeakCanary", "找到了leakingRef = " + leakingRef);
  ...
  void deduplicateGcRoots(Snapshot snapshot) {
    ...
    Log.d("LeakCanary", "filter before gcRoots.size() = " + gcRoots.size());
    ...
    Log.d("LeakCanary", "filter after gcRoots.size() = " + gcRoots.size());
  }
  ...
  private Instance findLeakingReference(String key, Snapshot snapshot) {
    // Snapshot  ClassObj 都是haha库的class，利用哈哈库来分析hprof文件
    String className = KeyedWeakReference.class.getName();
    Log.d("LeakCanary", "findLeakingReference className:"+className + ", key:"+key);
    ClassObj refClass = snapshot.findClass(className);
    if (refClass == null) {
      throw new IllegalStateException(
          "Could not find the " + KeyedWeakReference.class.getName() + " class in the heap dump.");
    }
    List<String> keysFound = new ArrayList<>();
    Log.d("LeakCanary", "获取className:"+className + " 的所有实例");
    for (Instance instance : refClass.getInstancesList()) {
      List<ClassInstance.FieldValue> values = classInstanceValues(instance);
      // key 是属于 KeyedWeakReference的field哦
      ...
      if (keyCandidate.equals(key)) {
        Log.d("LeakCanary", "根据uuid 生产的key找到了那个对象的实例Instance");
        // referent 是属于 KeyedWeakReference的父类 Reference的属性
  ...
  private AnalysisResult findLeakTrace(long analysisStartNanoTime, Snapshot snapshot,
    ...
    Log.d("LeakCanary", "已找到本节点，因为node中有属性parent，所以容易找到引用链");
