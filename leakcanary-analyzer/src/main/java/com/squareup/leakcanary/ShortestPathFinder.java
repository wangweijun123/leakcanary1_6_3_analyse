...
final class ShortestPathFinder {
  ...
  Result findPath(Snapshot snapshot, Instance leakingRef) {
    ...
    Log.d("LeakCanary", " 搜集所有的Gc root 大概有16万个对象");
    enqueueGcRoots(snapshot);

    boolean excludingKnownLeaks = false;
    LeakNode leakingNode = null;
    Log.d("LeakCanary", "toVisitQueue size="+toVisitQueue.size() +
            ", toVisitIfNoPathQueue size = "+toVisitIfNoPathQueue.size());
    while (!toVisitQueue.isEmpty() || !toVisitIfNoPathQueue.isEmpty()) {
      ...
      //Log.d("LeakCanary", "从队列中拿出一个节点, 获取instance与要查询的实例比较");
      // Termination
      if (node.instance == leakingRef) {
        Log.d("LeakCanary", "根据实例找到了泄漏的节点");
        leakingNode = node;
        break;
      }
      ...
      //Log.d("LeakCanary", "居然不是 ...");
      if (node.instance instanceof RootObj) {
        // 找到自己所引用的子节点吧,找到后添加到queue中去,继续便利
        // Log.d("LeakCanary", "访问根节点");
        visitRootObj(node);
      } else if (node.instance instanceof ClassObj) {
       // Log.d("LeakCanary", "访问ClassObj");
        visitClassObj(node);
      } else if (node.instance instanceof ClassInstance) {
       // Log.d("LeakCanary", "访问ClassInstance");
        visitClassInstance(node);
      } else if (node.instance instanceof ArrayInstance) {
       // Log.d("LeakCanary", "访问ArrayInstance");
        visitArrayInstance(node);
      } else {
        throw new IllegalStateException("Unexpected type for " + node.instance);
      }
    }
    Log.d("LeakCanary", "找到了node返回结果");
